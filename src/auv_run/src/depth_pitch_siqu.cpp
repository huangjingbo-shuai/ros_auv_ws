...
public:
    ...
    void stabilize() {
        ...
        if (!in_altitude_deadzone) {
            ...
            last_stable_pwm_ch3 = 5 + base_throttle + altitude_output;
            last_stable_pwm_ch4 = 5 + base_throttle + altitude_output;
        } else {
        ...
        if (in_altitude_deadzone) {
            ...
            target_pwm_ch3 = 5 + last_stable_pwm_ch3 - pitch_output;  // 前推进器
            target_pwm_ch4 = 5 + last_stable_pwm_ch4 + pitch_output;  // 后推进器
        } else {
            // 非死区：正常的混合控制
            target_pwm_ch3 = 8 + base_throttle + altitude_output - pitch_output;  // 前推进器
            target_pwm_ch4 = 8 + base_throttle + altitude_output + pitch_output;  // 后推进器
        }
        ...
        // PWM死区线性映射函数
        auto mapPWMWithDeadzone = [&](uint16_t raw_pwm) -> uint16_t {
            // 首先确保在有效范围内
            raw_pwm = std::max((uint16_t)1100, std::min((uint16_t)1900, raw_pwm));
            
            if (raw_pwm > 1500) {
                // 线性映射 [1501, 1900] -> [1500+pwm_deadzone, 1900]
                if (raw_pwm == 1500) return 1500;
                double ratio = (double)(raw_pwm - 1500) / (1900 - 1500);
                uint16_t mapped = 1500 + pwm_deadzone + (uint16_t)(ratio * (1900 - 1500 - pwm_deadzone));
                return std::min((uint16_t)1900, mapped);
            } else if (raw_pwm < 1500) {
                // 线性映射 [1100, 1499] -> [1100, 1500-pwm_deadzone]
                double ratio = (double)(raw_pwm - 1100) / (1500 - 1100);
                uint16_t mapped = 1100 + (uint16_t)(ratio * (1500 - pwm_deadzone - 1100));
                return std::max((uint16_t)1100, mapped);
            } else {
                return 1500; // 保持中性位置
            }
        };
        // 应用PWM死区映射
        target_pwm_ch3 = mapPWMWithDeadzone(target_pwm_ch3);
        target_pwm_ch4 = mapPWMWithDeadzone(target_pwm_ch4);
        ...
        controlMotors(target_pwm_ch3, target_pwm_ch4);
